...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    average_pop = [0.0 for _ in range(300)]
    ...
    average_pop = [pop_sum / numTrials for pop_sum in average_pop]

    pylab.figure()
    pylab.plot(range(300), average_pop)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        # Helper method used to mutate the resistances
        def generate_new_resistances():
            nresistances = self.resistances.copy()

            for k in nresistances.keys():
                if random.random() <= self.mutProb:
                    nresistances[k] = not nresistances[k]

            return nresistances

        # Tests for drugs that the virus has not resistance against
        working_drugs = [not self.isResistantTo(drug) for drug in activeDrugs]
        if all(working_drugs) and not len(working_drugs) == 0:
            raise NoChildException
        else:
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                new_resistances = generate_new_resistances()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for _ in range(numTrials):
        ...
        for time_step in range(150):
            average_pop[time_step] += patient.update()
            average_res[time_step] += patient.getResistPop(resistances.keys())
    ...
    average_pop = [pop_sum / numTrials for pop_sum in average_pop]
    average_res = [pop_sum / numTrials for pop_sum in average_res]

    print average_pop
    print average_res

    pylab.figure()
    pylab.plot(range(300), average_pop, 'b-', label='Avg Virus Population')
    pylab.plot(range(300), average_res, 'r--', label='Avg Resistant Population')
...
if __name__ == '__main__':
    ...
    simulationWithDrug(100, 1000, 0.1, 0.05, {'guttagonol': False}, 0.05, 5)
