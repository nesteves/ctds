...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for p in c2.members():
            point_list.append(p)
        self.add(Cluster(point_list, c1.pointType))
    ...
    def findClosest(self, linkage):
        ...
        for c1 in range(self.numClusters()):
            for c2 in range(self.numClusters()):
                if c1 != c2:
                    current_linkage = linkage(self.members[c1], self.members[c2])
                    if min_linkage is None or current_linkage < min_linkage:
                        best_pair = (self.members[c1], self.members[c2])
    ...
    def mergeOne(self, linkage):
        ...
        clusters_to_merge = self.findClosest(linkage)
        self.mergeClusters(clusters_to_merge[0], clusters_to_merge[1])
